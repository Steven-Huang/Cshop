...
/*
 | --------------------------------------------------------------------------
 | Author: Fusky  |  E-Mail: choggle2011@gmail.com  |  2016-04-06 09:35:46
 | --------------------------------------------------------------------------
...
class PublicAction extends Action {
    ...
    //退款报文
    public function SRReq($flowNo,$date,$charge,$amount,$originalOrderNo,$originalDate,$originalTime){
        $this->messageId='';
        $this->verId='1.0.0';
        // $this->messageId=$messageId;
        // $this->verId=$verId;
        ...
        //未加密报文(不能有任何其他字符，包括空格与回车符)
        $postData ='<CSRCEBankData><Message id="'.$this->messageId.'"><SRReq id="SRReq"><version>'.$this->verId.'</version><merchantNo>'.$this->merchantNo.'</merchantNo><instId>'.$this->instId.'</instId><certId>'.$this->certId.'</certId><flowNo>'.$flowNo.'</flowNo><date>'.$this->date.'</date><charge>'.$this->charge.'</charge><amount>'.$this->amount.'</amount><currency>'.$this->currency.'</currency><originalSerialNo>'.$this->originalOrderNo.'</originalSerialNo><originalDate>'.$this->originalDate.'</originalDate><originalTime>'.$this->originalTime.'</originalTime></SRReq></Message></CSRCEBankData>';
    
        //报文消息体MD5加密
        $sigNature=md5($postData.$this->key);
    
        //报文添加加密签名
        $postData ='<CSRCEBankData><Message id="'.$this->messageId.'"><SRReq id="SRReq"><version>'.$this->verId.'</version><merchantNo>'.$this->merchantNo.'</merchantNo><instId>'.$this->instId.'</instId><certId>'.$this->certId.'</certId><flowNo>'.$flowNo.'</flowNo><date>'.$this->date.'</date><charge>'.$this->charge.'</charge><amount>'.$this->amount.'</amount><currency>'.$this->currency.'</currency><originalSerialNo>'.$this->originalOrderNo.'</originalSerialNo><originalDate>'.$this->originalDate.'</originalDate><originalTime>'.$this->originalTime.'</originalTime></SRReq><Signature>'.$sigNature.'</Signature></Message></CSRCEBankData>';
